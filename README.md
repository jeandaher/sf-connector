...
👉 Ton architecture MuleSoft Case CRUD doit utiliser JWT Bearer (idéal) ou Client Credentials (simple).    
...
👉 Donc Informatica = OAuth Authorization Code + Refresh Token obligatoire.    

### 🧭 5. Tableau final : MuleSoft vs Informatica    
🔄 Comparaison des méthodes supportées    
Méthode	MuleSoft	Informatica	Commentaire    
Username + Password	✔	✔	À éviter partout    
OAuth Web Server (Auth Code)	✔	✔	MuleSoft : non adapté ; IICS : obligatoire    
OAuth Client Credentials	✔	❌	MuleSoft : simple M2M    
OAuth JWT Bearer	✔	❌	MuleSoft : enterprise-grade    


### 🏆 6. Recommandations finales pour ton architecture
